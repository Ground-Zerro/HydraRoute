...
- [🔄 Управление и перезапуск](#-управление-и-перезапуск)
...
> 💡 **Примечание:** домен `hr.net` больше **не используется** и полностью исключён из новой версии HydraRoute.
> Всё взаимодействие осуществляются напрямую через веб-интерфейс по адресу [http://192.168.1.1:2000](http://192.168.1.1:2000).
...
4. После установки откройте WebUI:
   [http://192.168.1.1:2000](http://192.168.1.1:2000)
   - Пароль по умолчанию: `keenetic`
   - После входа рекомендуется сменить пароль в настройках панели.
...
Пример:
```text
youtube.com,googlevideo.com/HydraRoute1st
openai.com,chatgpt.com/HydraRoute2nd
2ip.ru,2ip.io/HydraRoute3rd
```
...
```
> Группы отделяются пустыми строками, комментарии начинаются с `##`, отключенные CIDR с `#/`.
...
- Создаются автоматически при запуске HR.
- Если политика удалена пользователем — она пересоздаётся.
...
**Основные процессы:**
- Инициализация и очистка ipset при запуске.
- Синхронизация IPv4 и IPv6 ipset.
- Очистка iptables при запуске HR.
- Таймаут ipset: 6 часов.
- Удаление правил маршрутизации при остановке HR `hr stop`.

**WebUI:**
- Работает на порту `2000`.
- Авторизация через cookie `authenticated=1` (7 дней).
- Смена пароля доступна через WebUI.

---

## 🔄 Управление и перезапуск
...
hr restart
```
Эта команда пересоздаcт политики, ipset, iptables и перезапустит AdGuard Home.
...
curl -Ls "https://ground-zerro.github.io/release/keenetic/hr-uninstall.sh" | sh
```
Удаляет все iptables/ipset, политики, зависимости, службы, файлы HydraRoute и AdGuardHome.
